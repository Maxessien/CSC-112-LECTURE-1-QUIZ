...
function runTimer(){
    const intervalId = setInterval(()=>{
                            ...
                            if (time===0) {
                                ...
                                fetch('https://raw.githubusercontent.com/Maxessien/Test-API-Fetch-/main/answer.json').then(res=>{
                                    return res.json()
                                }).then(data=>{
                                    answers = data
                                    displayResults()
...
function loadQuestions(){
    if (currentQuestion === quizDatas.length - 1) {
        elementDisplay([{target: nextBtn, action: 'remove'},
            {target: submitBtn, action: 'add'}])
    }else if (currentQuestion < quizDatas.length - 1) {
        elementDisplay([{target: nextBtn, action: 'add'},
...
function createQuestions(){
    quizDatas.forEach((quizData, index)=>{
        ...
        questionsHtml.push(
            ...
            <ul class="options">
                ${
                    quizData.options.map((element, i)=>{
                        return `<li class="options-list">
                                    <input type="radio" class="no-display current-inputs" value="${element}" name="question${currentQuestion+1}options" id="opt${i+1}">
...
function selectedOption(){
    ...
    currentInputs.forEach(input=>{
        if (document.querySelector(`label[for="${input.id}"]`).classList.contains('selected')) {
    ...
    allInputs.forEach(input=>{
        input.addEventListener('change', ()=>{
            ...
            allInputs.forEach(inputs=>{
                document.querySelector(`label[for="${inputs.id}"]`).classList.remove('selected')
            })
            if (input.checked) {
...
function loadNavigation(){
    quizNavigation.innerHTML = quizDatas.map((quizData, index)=>{
